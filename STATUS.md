...
## Current phase: v1.1 (Sonic Variety) — 6 sonic items merged to `master` (2026-05-29) + ears-on verified; ship-prep batch (crash logging, Inno installer, drag-out WAV) landed on branch `v1.1-ship-prep`. ctest → 5/5 pass; VST3 + Standalone build clean. `master` is 8 commits ahead of origin (unpushed by request). Next: install Inno Setup 6 and actually compile the installer (`scripts\build_installer.bat`); then DAW validation matrix + release tag.
...
### v1.1 ship-prep (2026-05-29, branch `v1.1-ship-prep`)
- ✅ **Crash logging** — `Source/CrashLog.{h,cpp}`: `juce::SystemStats::setApplicationCrashHandler` writes a timestamped report (app/JUCE/OS/CPU + stack backtrace) to `%APPDATA%\KickAss\Logs\`. Installed **Standalone-only** (`wrapperType == wrapperType_Standalone`) so we don't hijack a host's crash handler — plugin-host crash capture is deferred. Pure pieces (path/format/write) factored out and tested headlessly via new `KickAss_CrashTests` target (`Tests/CrashLogTest.cpp`). The handler itself is not unit-testable without faulting.
- ✅ **Drag-out WAV** — footer **DRAG WAV ↗** affordance: drag it onto the desktop / a DAW track to drop a freshly-rendered 24-bit/48k stereo WAV of the current patch. Editor is now a `juce::DragAndDropContainer`; `DragOutButton::mouseDrag` → `performExternalDragDropOfFiles`. Render path extracted to `KickAssProcessor::renderToWavFile()` (shared with EXPORT WAV; duration logic shared via `computeRenderDurationMs()`), tested headlessly via new `KickAss_RenderTests` target (`Tests/RenderWavTest.cpp` — valid/readable/non-silent/finite WAV across all factory presets).
- ✅ **Inno Setup installer** — `installer/KickAss.iss` (+ `scripts/build_installer.bat`): installs VST3 → `Common Files\VST3\`, Standalone → `Program Files\KickAss\`, with components/uninstall/desktop-shortcut. **NOT yet compiled** — Inno Setup 6 isn't installed on this machine; the `.bat` fails gracefully with install instructions. Artefact source paths verified to exist. TODO: install Inno Setup 6, run the bat, confirm `installer\Output\KickAss-1.1.0-Setup.exe`. (Installer version string is 1.1.0; CMake project VERSION still 0.1.0 — bump when tagging.)
